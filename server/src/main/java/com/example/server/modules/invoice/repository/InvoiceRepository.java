...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
    ...
    // Dashboard Metrics Methods
       ...
       @Modifying
       @Query("UPDATE Invoice i SET i.status = :newStatus, i.updateOf = :now " +
              "WHERE i.status IN ('ISSUED', 'SENT', 'PARTIALLY_PAID') " +
              "AND i.dueAt < :now")
       int updateOverdueInvoices(@Param("newStatus") InvoiceStatus newStatus, 
                            @Param("now") LocalDateTime now);

       @Query("SELECT i FROM Invoice i " +
       "WHERE i.company.id = :companyId " +
       "AND i.status IN ('DRAFT', 'ISSUED', 'SENT', 'PARTIALLY_PAID') " +
       "ORDER BY i.dueAt ASC")
       List<Invoice> findPendingInvoices(@Param("companyId") Long companyId);

...
export function TransactionDetail({ transaction: t, onClose }: TransactionDetailProps) {
  ...
  return (
    <Modal title="Transaction detail" onClose={onClose} size="md">
      <div className="flex flex-col gap-4 -mx-6 -mt-1">
 
        {/* Amount hero */}
        <div className={`mx-6 rounded-xl px-5 py-6 flex flex-col gap-2 ${isCredit ? 'bg-emerald-50' : 'bg-red-50'}`}>
        ...
        <div className="px-6">
          ...
          {t.invoiceNumber && <Row label="Invoice"  value={t.invoiceNumber} />}
          {t.customerName  && <Row label="Customer" value={t.customerName} />}
        </div>
      </div>
    </Modal>
  );
